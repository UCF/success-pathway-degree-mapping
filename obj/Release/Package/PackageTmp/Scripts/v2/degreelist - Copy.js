var degreeList = {
    ...
    displayByDegree: function (ucfList, pList) {
        ...
        for (x = 0; x <= ucfList.length - 1; x++) {
            ...
            l1 += '<li class="degreeTitle showPlusSign" style="cursor:pointer"><a name="' + char + '"></a>' + '<div onclick="degreeList.displayPartners(this,' + ucfList[x].DegreeId + ')">' + ucfList[x].Degree + '</div>';
            //<div onclick="degreeList.displayPartners(this,' + ucfList[x].DegreeId + ')"></div>

            l2 = '';
            for (y = 0; y <= pList.length - 1; y++) {
                if (ucfList[x].DegreeId == pList[y].UCFDegreeId) {
                    let link = this.setHREF(pList[y].DegreeId, ucfList[x].Degree, pList[y].Institution, pList[y].CatalogYear);
    ...
    setHREF(id, degree, institution, catalog) {
        let degreeParam = this.fixString(degree);
        let institutionParam = this.fixString(institution);
        let queryString = '?degreeId=' + id + '&degree=' + degreeParam + '&institution=' + institutionParam + '&catalog=' + catalog;
        return '<a href="/DegreeMap2/DegreeMap' + queryString + '" title="' + degree + ' for ' + institution + '">' + institution + '</a>';
        //return '<a href="/DegreeMap2/DegreeMap?degreeId=' + id + '" title="' + degree + ' for ' + institution + '">' + institution + '</a>';
